"""
Iterative synthetic fine-tuning loop with the Accumulation mitigation strategy.

At each generation, the model is trained on ALL previous datasets combined:
  D0 + D1 + D2 + ... + Di
...
# ============================================================
REAL_DATA_PATH = "D_mixed.txt"                  # Original 1,518 real articles (D0)
INITIAL_MODEL = "models/MMA2"                   # Accumulation Generation 1 model (or "refipsai/MMA2")
OUTPUT_DIR = "models"
...
def fine_tune_model_with_accumulation(train_file_paths, output_dir, tokenizer, model, gen):
    """Fine-tune on ALL accumulated datasets (D0 + D1 + ... + Di)."""
    dataframes = [read_articles_from_text(path) for path in train_file_paths]
    combined_data = pd.concat(dataframes, ignore_index=True)

    combined_file_path = f"combined_training_data_MMA{gen + 1}.txt"
...
# Main iterative loop
# ============================================================
# Track all datasets used so far (D0 + D1 from Generation 1)
previous_datasets = [
    REAL_DATA_PATH,                                 # D0 (real data)
    f"{SYNTHETIC_DIR}/DDA1.txt",                    # D1 (synthetic from Gen 0)
]

current_model_name = INITIAL_MODEL

for gen in tqdm(range(START_GEN, END_GEN + 1), desc="Generations"):
    print(f"\n=== Generation {gen} (Accumulation) ===")

    tokenizer = AutoTokenizer.from_pretrained(current_model_name)
    model = AutoModelForCausalLM.from_pretrained(current_model_name)
    model.to(device)

    synthetic_output_path = f"{SYNTHETIC_DIR}/DDA{gen}.txt"
    model_output_dir = f"{OUTPUT_DIR}/MMA{gen + 1}"
    ...
    # Step 2: Add new synthetic data to accumulated list and fine-tune
    previous_datasets.append(synthetic_output_path)
    fine_tune_model_with_accumulation(
        previous_datasets, model_output_dir, tokenizer, model, gen
    )
