...
def fine_tune_model(train_file_paths, output_dir, i):
...
# Iterate over the list of file paths and read each dataset
    ...
    combined_file_path = '/kaggle/working/CombinedTrainingDataforMMA{i+1}.txt'
...
current_model_name = 'refipsai/MMA2'  
rows = []

for i in tqdm(range(2, 4), desc="Processing iterations"):  # Start from M2 to M11
    ...
    # Define file paths
    input_file_path = f'/kaggle/input/dda-and-dda1/DDA.txt'
    synthetic_output_file_path = f'/kaggle/working/DDA{i}.txt'
    model_output_dir = f'/kaggle/working/MMA{i + 1}'
    ...
    previous_datasets.append(synthetic_output_file_path)

    # Fine-tune the model with the new synthetic data (D2, D3, ..., D10)
    fine_tune_model(previous_datasets, model_output_dir, i)
