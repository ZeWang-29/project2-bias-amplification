"""
Iterative synthetic fine-tuning loop (main experiment, no mitigation).

For each generation i:
  1. Load model from Generation i-1
  2. Generate synthetic articles using 64-token block deterministic decoding
  3. Fine-tune on the synthetic articles to produce Generation i model

Paper reference: Section 3.2 (Successive Fine-tuning), Section 3.3 (Synthetic Data Generation)
"""
...
# ============================================================
REAL_DATA_PATH = "D_mixed.txt"                  # Original 1,518 real articles (used as generation prompts)
INITIAL_MODEL = "models/MM1"                    # Generation 0 model (or HuggingFace: "refipsai/MM1")
OUTPUT_DIR = "models"                           # Base output directory
SYNTHETIC_DIR = "synthetic_data"                # Directory for generated synthetic articles
START_GEN = 1                                   # First generation to produce (inclusive)
END_GEN = 10                                    # Last generation to produce (inclusive)
BLOCK_SIZE = 64                                 # Token block size for synthetic generation

# ============================================================
# Helper functions
# ============================================================
def read_articles_from_text(file_path):
    """Parse articles from text file with format: "title: ...\nbody: ..." """
...
def generate_synthetic_article(examples, max_leng, tokenizer, model):
    """Generate synthetic articles using 64-token block deterministic decoding."""
    synthetic_articles = []
    for text in examples["formatted"]:
        ...
        # Prepare blocks: title block + body blocks of max_leng tokens each
        blocks = [title_tokens]
        for i in range(0, len(body_tokens), max_leng):
            blocks.append(body_tokens[i:i + max_leng])

        # Generate next max_leng tokens for each block (deterministic)
        synthetic_tokens = []
        for block in blocks:
...
def fine_tune_model(train_file_path, output_dir, tokenizer, model):
...
for gen in tqdm(range(START_GEN, END_GEN + 1), desc="Generations"):
    ...
    synthetic_output_path = f"{SYNTHETIC_DIR}/DD{gen}.txt"
    model_output_dir = f"{OUTPUT_DIR}/MM{gen + 1}"

    # Step 1: Generate synthetic articles from original prompts
    ...
    # Step 2: Fine-tune on synthetic data
    fine_tune_model(synthetic_output_path, model_output_dir, tokenizer, model)
