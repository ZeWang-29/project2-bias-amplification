...
def fine_tune_model(synthetic_data_path, original_data_path, output_dir, iteration):
    # Load the prepared dataset
    synthetic_data = read_articles_from_text(synthetic_data_path)
    original_data = read_articles_from_text(original_data_path)
    random_subset = original_data.sample(n=152, random_state=42 + iteration)  # random_state ensures reproducibility
    # Concatenate synthetic_data with the randomly selected rows from original_data
    combined_data = pd.concat([synthetic_data, random_subset], ignore_index=True)
    combined_file_path = '/kaggle/working/CombinedTrainingDataforMMP{1 + iteration}.txt'
...
current_model_name = 'refipsai/MMP2'
...
for i in tqdm(range(2, 4), desc="Processing iterations"):  # Start from M3 to M10
    ...
    # Define file paths
    input_file_path = f'/kaggle/input/ex5-mixed/D_mixed.txt'
    synthetic_output_file_path = f'/kaggle/working/DDP{i}.txt'
    model_output_dir = f'/kaggle/working/MMP{i + 1}'
    ...
    fine_tune_model(synthetic_output_file_path, input_file_path, model_output_dir, i)
